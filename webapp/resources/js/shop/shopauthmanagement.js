$(function(){
	//列出该店铺下的所有授权信息的url
	var listUrl = '/o2o/shopadmin/listshopauthmapbyshop?pageIndex=1&&pageSize=3'
	//修改授权信息的url
			...
			shopauthList.map(function(item,index){
				...
				if(item.titleFlag != 0){
					...
					tempHtml += '<div class="col-20">' + item.title
						...
						+ item.employee.userId + '" data-auth-id="' +item.shopAuthId +'">编辑</a>'
						+ '<a href="#" class="status" data-auth-id="'
						+ item.shopAuthId + '" data-status="'+contraryStatus+'">'
						+ textOp + '</a>'
						+'</div>' 
				}else{
					//若为店家，则不允许操作
